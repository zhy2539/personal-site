...
const PENDING_FILE = path.join(DATA_DIR, "skills-pending.json");
...
const SEARCH_QUERIES = [
  ...
  "topic:cursor-skill stars:>5",
  "agent skill SKILL.md in:path",
];
...
async function searchGitHub(query: string): Promise<GitHubRepo[]> {
  ...
  const headers: Record<string, string> = {
    ...
    "User-Agent": "personal-site-discover-skills",
  };
...
function repoToResource(repo: GitHubRepo): ResourceItem {
  const tags = ["GitHub", "Skill", ...(repo.topics ?? []).slice(0, 3)];
  return {
    id: slugify(repo.full_name),
    name: repo.full_name.split("/").pop() ?? repo.full_name,
    description: repo.description ?? `Cursor/Agent Skill 相关仓库：${repo.full_name}`,
    url: repo.html_url,
    tags: [...new Set(tags)],
...
async function main() {
  console.log("🔍 发现 Skill 候选资源…");

  const existing = await readJson<ResourceItem[]>(SKILLS_FILE, []);
